...
- no Git index lock or staged change is present, and runtime source is clean; and
...
fingerprint atomically only after `brain.service` reports active and bounded read-only GET checks confirm supported summary and operations responses. The fingerprint hashes committed files under `scripts/`, `memory/brain/view/`, and `systemd/user/`. Unstaged operator guidance, private ledgers, handoffs and ignored generated snapshots are data or maintenance context, not imported runtime source, and remain untouched. Any staged change still defers deployment; no-runtime-source-change commits do not restart it. Reflog/ref
duplicates and unrelated file or ledger writes therefore do not restart Brain.
...
If no later ref update is expected, invoke the helper once manually after the
source is clean. Its durable state lives outside the checkout so recording a
fingerprint never makes the watched source dirty.
...
them only after runtime source and the index are clean, `HEAD` is the intended local `main`
commit, and the parent has stopped the old manual process using its approved
procedure.
