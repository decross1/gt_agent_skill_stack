...
def canonicalize_agent(raw_id: str | None) -> str | None:
    ...
    if r == "coordinator":
        return "coordinator"
    if r in ("integrator", "primary-session/integrator"):
        return "integrator"
    if r.startswith("workflow:"):
        # Dynamic-Workflow limb ids ("workflow:wf_641dcae3-bdd/limb-a"): collapse
        # the per-run wf-id, keep the role suffix so recurring roles aggregate
        # across workflows ("workflow:limb-a").
        rest = r.split(":", 1)[1]
        role = rest.split("/", 1)[1] if "/" in rest else rest
        return f"workflow:{role}"
